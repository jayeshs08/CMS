...
export default function ViewStatus({setShowModal}) {
  ...
  const handleTicketCancel = (ticketnum) => {
    ...
    axios
      ...
      .catch(error => {
        console.log("Error in executing delete button in ticketstatus");
      });
  };
  ...
  return (
    <div className="ticketstatus h-[500px] mt-[40px]">
    <Toaster/>
                  ...
                  {ticketData.map(ticket => (
                    <tr key={ticket.ticketNum}>
                      ...
                      <td className={`px-6 py-4 text-sm text-gray-800 whitespace-nowrap ${
                        ...
                        <td className="px-6 py-4 text-sm text-gray-800 whitespace-nowrap"><button type='button' onClick={handleViewClick}>View</button></td>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-800 whitespace-nowrap">
                        <div className='flex items-center'>
                          <svg  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 hover:bg-slate-200 hover:rounded" onClick={()=>{handleEdit()}} >
                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125"/>
                          </svg>
                            <div className='mx-[5%] text-xl' >
                            / 
                            </div>
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 hover:bg-slate-200 hover:rounded" onClick={()=>handleClick()} >
    ...
    {ticketData.map(ticket=>(
      <tr key={ticket.ticketNum}>
    {isEdit && <EditForm dataProp={ticket} onFormSubmit={handleFormSubmit} cancelEdit={cancelEdit}/>}
    </tr>))}

    {ticketData.map(ticket=>(
      <tr key={ticket.ticketNum}>
    {isCancel && <TicketCancel dataProp={ticket} onCancel={handleTicketCancel} onClose={handleClose}  />}
    </tr>))}
